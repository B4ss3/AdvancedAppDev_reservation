<script>
  ...
  const deleteApartment = async () => {
    const confirmDelete = confirm('Are you sure you want to delete this apartment?');

    if (confirmDelete) {
      try {
        const response = await fetch(`/api/apartments/${data.apartmentId}`, { method: 'DELETE' });
...
<article class="max-w-lg m-auto">
  ...
  <form class="px-2 mb-2 flex flex-col" method="POST" use:enhance>
    ...
    <button class="btn variant-filled mt-2" type="submit">Update</button>
    <button class="btn variant-filled mt-2" on:click={deleteApartment}>Delete</button>
  </form>
</article>